...
exports.createAuction = async (req, res) => {
  try {
    ...
    const changesItem = await Item.findByIdAndUpdate(itemId, {
      status: 'listed'
    })
...
exports.getUsersBiddings = async (req, res) => {
  const { id } = res.locals.payload
  let response = await Auction.find({ status: 'ongoing' })
    .populate('itemId')
    .lean()

  for (const auction of response) {
    auction.inAuction = false
    const biddings = await Bidding.find({ auctionId: auction._id })
    auction.biddings = biddings
    auction.biddings.forEach((bidding) => {
      if (bidding.userId.toString() === id) {
